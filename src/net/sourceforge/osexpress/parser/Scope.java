...
public class Scope implements ExpressParserTokenTypes, Cloneable
{
	...
	public void addId(String id, int type)
	{
		 ...
		 * Enumeration ids are recorded in the parent scope for visibility reasons
		 * (it should be visible out of the TYPE in which it is defined
		 */
		i = (Integer) idtable.get(id.toLowerCase());
		if (i == null)
			idtable.put(id.toLowerCase(), new Integer(type));
		else
	...
	public void addEnumerationType(EnumerationType et)
	{
		...
		/*
		 * to record enumeration ids out of their original scope (TYPE) Also records
		 * the enumerations with the type where they are defined so they can be
		 * retrieved when processing REFERENCE clauses
		 */
		sz = et.getCount();
		for (i = 0; i < sz; i++)
		{
			...
			if (ii == null)
				idtable.put(id.toLowerCase(), new Integer(ENUMERATION_IDENT));
			else if (ii.intValue() != ENUMERATION_IDENT)
				System.err.println("Warning: id \"" + id + "\" already defined");
		}
	...
	public int searchId(String id)
	{
		...
		/*
		 * search the given id in this scope's table, in the entity inheritance tree
		 * and in all the scopes tree if not found. Because of part 10.2.2 of ISO
		 * 10303-11:1994, we have to consider the case of redeclaration of attribute
		 * without hiding the entity with the same name in the outer scope. So if an
		 * id is declared as en entity and an attribute, an ENTITY_ATTR_IDENT token
		 * is generated. same for TYPE_ATTR_IDENT, ENTITY_VAR_IDENT, TYPE_VAR_IDENT
				...
				if (i != null)
				{
					if (i.intValue() == ATTRIBUTE_IDENT)
					{
						if (parent != null)
						{
							...
							if (it == ENTITY_IDENT)
								return ENTITY_ATTR_IDENT;
							else if (it == TYPE_IDENT)
								return TYPE_ATTR_IDENT;
						}
					}
					else if (i.intValue() == VARIABLE_IDENT)
					{
						if (parent != null)
						{
							it = parent.searchId(id);
							if (it == ENTITY_IDENT)
								return ENTITY_VAR_IDENT;
							else if (it == TYPE_IDENT)
								return TYPE_VAR_IDENT;
						}
					}
					else if (i.intValue() == PARAMETER_IDENT)
					{
						if (parent != null)
						{
							it = parent.searchId(id);
							if (it == ENTITY_IDENT)
								return ENTITY_PARAM_IDENT;
							else if (it == TYPE_IDENT)
								return TYPE_PARAM_IDENT;
						}
					}
					return i.intValue();
				}
	...
	public Vector getReferencedExternals()
	{
		...
		while (e.hasMoreElements())
		{
			...
			if (type == CONSTANT_IDENT || type == ENTITY_IDENT || type == FUNCTION_IDENT
					|| type == PROCEDURE_IDENT || type == TYPE_IDENT)
			{
				if (res == null)
					res = new Vector();
	...
	{
		/*
		 * add ids of used and referenced elements of other schemas to this schema's
		 * scope
		 */
		int sz, i, sz2, j, eid;
		ExternalId ei;
		...
		if (!extdone)
		{
			if (externals != null)
			{
				...
				for (i = 0; i < sz; i++)
				{
					...
					if (s == null)
						System.err.println("Warning: REFERENCED or USED schema \"" + ei.schema
								+ "\" does not exist in the parsed file");
					else
					{
						/*
						 * schema has to be processed before since you can USE or REFERENCE
						 * elements that the schema USEs itself
						 */
						s.processExternals(schemas);
						eid = s.searchId(ei.id);
						if (eid == IDENT)
							System.err.println("Warning: external id \"" + ei.id + "\" not found in schema \""
									+ ei.schema + "\"");
						else
						{
							rn = ei.rename;
							/*
							 * idtable.put is used instead of addId because some externals ids
							 * may be imported twice or more, so it would throw double
							 * definition errors
							 */
							if (rn != null)
								idtable.put(rn.toLowerCase(), new Integer(eid));
							else
								idtable.put(ei.id.toLowerCase(), new Integer(eid));
							...
							if (eid == ENTITY_IDENT)
							{
								try
								{
									...
									/*
									 * we clone the Scope because we have to assign a new parent
									 * to it (this), but the original Scope should keep its parent
									 * attribute safe
									 */
								}
							...
							else if (eid == TYPE_IDENT)
							{
								...
								if (et != null)
								{
									...
									if (et.basedOnName != null)
									{
										...
										{
											if (!ext.extensible)
												System.err.println("Warning: enumeration type \"" + et.typename
														+ "\" based on non extensible enumeration \"" + et.basedOnName + "\"");
											et.extendWith(ext);
										}
										else
											System.err.println("Warning: extensible enumeration \"" + et.basedOnName
													+ "\" not found");
									}
			...
			{
				/*
				 * Here we manage REFERENCES to all referencable elements of a schema
				 * (when no element is specified in the REFERENCE clause)
				 */
				...
				for (i = 0; i < sz; i++)
				{
					...
					if (s == null)
						System.err.println("Warning: REFERENCED schema \""
								+ (String) allReferencedSchemas.elementAt(i)
								+ "\" does not exist in the parsed file");
					else
					{
									...
									if (et != null)
									{
										...
										if (et.basedOnName != null)
										{
											...
											if (ext != null)
											{
												if (!ext.extensible)
													System.err
															.println("Warning: enumeration type \"" + et.typename
																	+ "\" based on non extensible enumeration \"" + et.basedOnName
																	+ "\"");
												et.extendWith(ext);
											}
			...
			{
				/*
				 * Here we manage USEs of all useable elements of a schema (when no
				 * element is specified in the USE clause)
				 */
				...
				for (i = 0; i < sz; i++)
				{
					...
					if (s == null)
						System.err.println("Warning: USED schema \"" + (String) allUsedSchemas.elementAt(i)
								+ "\" does not exist in the parsed file");
					else
					{
